...
fun ProductsView(
    ...
    Box(
        ...
        if(state.products.isEmpty()){
            Text(
                text = stringResource(R.string.product_view_empty),
                modifier = Modifier
                    .fillMaxWidth()
        ...
        Column {
            CustomToolbar(
                title = stringResource(R.string.product_view_title),
                hasBackButton = true,
                onBackClick = {
                    event(GroceryPlanEvent.BackPress)
                }
            )
            LazyColumn(
                modifier = Modifier
                ...
                items(state.products) { product ->
                    ProductsItem(
                        ...
                        onChangeQuantity = { id, quantity ->
                            event(GroceryPlanEvent.UpdateProductQuantity(productId = id, quantity = quantity))
                        },
                        onDelete = {
                            event(GroceryPlanEvent.OpenDeleteProductDialog(it))
                        },
                    )
                }
            }
        ...
        PrimaryButton(
            modifier = Modifier
                ...
                .padding(XLSpacing),
            text = stringResource(R.string.product_view_create_button)
        ) {
            event(GroceryPlanEvent.OpenNewProductDialog(state.selectedPurchaseId))
        }
    }
}
